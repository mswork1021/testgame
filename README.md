...
### 装備モーダルのボタンがリアルタイム反映されない

**症状:**
- 装備モーダル内の4つのボタン（強化、サブステ追加、サブステ値抽選、種類変更）を押しても**無反応**に見える
- トーストメッセージも表示されない
- **ただしデータ自体は更新されている**（タブを切り替えると変更が反映される）

**試した修正（すべて失敗）:**
1. インラインonclick属性 (`onclick="window.TapQuest.ui.onEnhanceItem()"`)
2. イベント委譲方式（bindEventsでclickイベントを捕捉）
3. touchend + click の両方対応
4. `self = this` でのthisバインディング修正
5. data-action属性 + イベント委譲方式（現在の実装）

**関連ファイル:**
- `js/ui.js` - 行400-442付近: イベント委譲の設定
- `js/ui.js` - 行1541-1574付近: ボタンHTML生成（data-action属性）
- `js/ui.js` - 行1617-1693付近: ハンドラー関数（onEnhanceItem, onValueReroll等）
- `js/ui.js` - 行1890-1918付近: showToast関数

**推測される原因:**
- モバイルブラウザ特有のイベント処理問題
- z-indexやポインターイベントの衝突
- innerHTMLで生成した要素へのイベントが届いていない可能性
- キャッシュが完全にクリアされていない可能性

**次のセッションで試すべきこと:**
1. ブラウザのキャッシュを完全クリア（設定→サイトデータ削除）
2. コンソールログで `Equipment action:` が出力されるか確認
3. ボタンにtouchstartイベントを追加（touchendではなく）
4. モーダル自体のz-indexやpointer-events確認
5. ボタン要素がDOM上に正しく存在するかDevToolsで確認
...
- **スキルポイント(SP)**: 転生時に獲得（ステージ50ごとに+1SP）
...
### 宝箱＆ラッキータイム (v2.31)
- **宝箱自動収集**: ステージ進行中に宝箱が自動でストックされる
- **一括開封**: 貯めた宝箱を好きなタイミングで一括開封
- **宝箱報酬**: ゴールド、ジェム、ソウル、レア装備、スキルリセットなど
- **ラッキータイム**: ストック可能！好きなタイミングで発動可能
- **レインボーボーダー**: ラッキータイム中はモンスター画面が虹色に光る
- **専用エフェクト**: 獲得通知、一括開封結果、ラッキータイム発動時の演出
...
- **v2.32**: バトル画面拡張モード（下部パネル最小化でタップ領域拡大）
- **v2.31**: 宝箱自動収集・ストック方式、ラッキータイム使い放題、レインボーボーダー
